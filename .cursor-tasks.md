...
* [ ] Define Database: `Cultures` table schema (CultureID PK, CultureName, Region, PrimaryLanguage, Description, LastUpdated).
* [ ] Define Database: `CulturalNorms` table schema (NormID PK, CultureID FK, Category [e.g., Greeting, Dining, Business, Gesture, SensitiveTopic], SubCategory, Description, DoBehavior, DontBehavior, Explanation, SeverityLevel, LastUpdated).
* [ ] Define Database: `Idioms` table schema (IdiomID PK, CultureID FK, Language, Phrase, LiteralTranslation, Meaning, UsageExamples, ContextNotes, PolitenessLevel, LastUpdated).
* [ ] Define Database: `Scenarios` table schema (ScenarioID PK, CultureID FK, Category, Title, SituationDescription, ImageURL_optional, LastUpdated).
* [ ] Define Database: `ScenarioChoices` table schema (ChoiceID PK, ScenarioID FK, ChoiceText, IsCorrectChoice, FeedbackText, ConsequenceDescription).
* [ ] Define Database: `UserPreferences` table schema (PreferenceID PK, UserID FK, CultureID FK, NormID FK, ReminderEnabled BOOLEAN).
* [ ] Setup: Choose database system (e.g., PostgreSQL, MySQL, MongoDB).
* [ ] Implement Backend: CRUD API endpoints for `Cultures` (MVP: ensure GET exists for app).
    * [ ] Implement Backend: `POST /admin/cultures` (Create) (Lower priority for MVP if manually populating)
    * [ ] Implement Backend: `GET /admin/cultures` (Read List) (MVP: ensure non-admin GET /cultures exists)
    * [ ] Implement Backend: `GET /admin/cultures/{id}` (Read One)
    * [ ] Implement Backend: `PUT /admin/cultures/{id}` (Update) (Lower priority for MVP)
    * [ ] Implement Backend: `DELETE /admin/cultures/{id}` (Delete) (Lower priority for MVP)
* [ ] Implement Backend: CRUD API endpoints for `CulturalNorms` (scoped by CultureID) (MVP: ensure GET exists for app).
* [ ] Implement Backend: CRUD API endpoints for `Idioms` (scoped by CultureID) (MVP: ensure GET exists for app).
* [ ] Implement Backend: CRUD API endpoints for `Scenarios` and `ScenarioChoices` (scoped by CultureID) (Lower priority for MVP).
...
* [ ] Task: Populate initial CKB with data for 2-3 diverse cultures for MVP testing and demo. (CRITICAL for MVP)
...
* [ ] Implement Backend: API endpoint (`GET /cultures`) to list all available cultures for user selection.
* [ ] Implement Backend: API endpoint (`GET /cultures/{cultureId}/briefing`) to fetch all relevant `CulturalNorms` for a full briefing.
* [ ] Implement Backend: API endpoint (`GET /cultures/{cultureId}/idioms?search={query}`) to search idioms for a culture.
* [ ] Implement Backend: API endpoint (`GET /cultures/{cultureId}/idioms/{idiomId}`) to fetch specific idiom details.
* [ ] Implement Backend: API endpoint (`GET /cultures/{cultureId}/scenarios?category={category}`) to list scenarios (Lower priority for MVP if scenarios deferred).
* [ ] Implement Backend: API endpoint (`GET /scenarios/{scenarioId}`) to fetch scenario details including choices (Lower priority for MVP).
* [ ] Implement Backend: API endpoint (`POST /scenarios/{scenarioId}/submit`) to evaluate a user's choice for a scenario (Lower priority for MVP).
* [ ] Task: Implement database indexing on frequently queried CKB fields (CultureID, Category, search terms).
* [ ] Task: Implement caching strategy for CKB data (e.g., Redis) to improve performance for common requests (Consider for MVP if performance is an issue).
...
* [ ] Design UI: Screen for selecting a culture (e.g., searchable list - start with list for MVP).
* [ ] Implement Client: Fetch list of available cultures from `GET /cultures` endpoint.
* [ ] Implement Client: Display list of cultures in a user-friendly way (e.g., sort alphabetically).
* [ ] Implement Client: Allow user to tap/select a culture to view its briefing.
* [ ] Implement Client: Store the currently selected culture in app state.
...
* [ ] Design UI: Briefing screen with clear sections for different cultural categories (e.g., using tabs, accordions, or a scrollable list of cards).
* [ ] Implement Client: When a culture is selected, fetch its detailed briefing data from `GET /cultures/{cultureId}/briefing`.
* [ ] Implement Client: Dynamically render briefing content based on fetched data, organized by `CulturalNorms.Category`.
    * [ ] Implement Client: Display "Greetings" norms.
    * [ ] Implement Client: Display "Dining Etiquette" norms.
    * [ ] Implement Client: Display "Business Protocols" norms.
    * [ ] Implement Client: Display "Gestures (Do's and Don'ts)" norms.
    * [ ] Implement Client: Display "Sensitive Topics to Avoid" norms.
    * [ ] Implement Client: Display other relevant categories from CKB for demo cultures.
* [ ] Implement Client: Ensure text is well-formatted, readable, and supports rich text if applicable (e.g., bolding, bullet points from CKB).
* [ ] Implement Client: Display "Last Updated" timestamp for the briefing content.
* [ ] Design UI: Visual cues for "Do" vs. "Don't" behaviors (e.g., icons, color coding).
* [ ] Implement Client: Handle cases where specific information for a sub-section is not available.
* [ ] Design UI: "Save for Offline Access" button (placeholder, functionality deferred for MVP).
* [ ] Design UI: "Set Reminder for this Norm" button next to specific norms (functionality deferred for MVP, links to Epic 9).
...
* [ ] Design UI: Screen for text analysis with a multi-line text input area.
* [ ] Design UI: Dropdown or selector to choose the target culture for analysis (pre-fill with last selected culture if any, from demo cultures).
* [ ] Design UI: Option (e.g., toggle, radio buttons) to specify if the text is "My Text" (for proactive advice) or "Their Text" (for interpretation aid).
* [ ] Implement Client: Component for text input and culture/origin selection.
* [ ] Implement Client: "Analyze Text" button.
* [ ] Implement Client: On "Analyze Text" click, send text, target culture ID, and text origin to backend.
...
* [ ] Design UI: Display area for AI feedback. This could involve:
    * [ ] Design UI: Highlighting specific phrases in the original text.
    * [ ] Design UI: Annotations or pop-ups next to highlighted text.
    * [ ] Design UI: A summary section for overall feedback and suggestions.
    * [ ] Design UI: Clear distinction between identified issues and suggested improvements.
* [ ] Implement Backend: API endpoint (`POST /analysis/text`) to receive text, culture ID, and origin.
* [ ] **AI Integration Point (Backend):** (CRITICAL for MVP)
    * [ ] Task: Pre-process input text (cleaning, tokenization).
    * [ ] Task for AI Model: Call sentiment/tone analysis model, calibrated for the target demo culture(s).
    * [ ] Task for AI Model: Call idiom detection model for the target demo culture(s).
    * [ ] Task for AI Model: Call cultural misstep detection model for the target demo culture(s).
    * [ ] Task for AI Model: Call alternative phrasing suggestion model for the target demo culture(s).
    * [ ] Task: Consolidate results from various AI model components. (Focus on clear, actionable feedback for MVP).
* [ ] Implement Backend: Format AI analysis results into a structured JSON response for the client. Include identified issues (type, location in text, severity, explanation) and suggestions.
* [ ] Implement Client: Parse AI feedback from backend.
* [ ] Implement Client: Render AI feedback in the designed UI, linking annotations to text.
* [ ] Implement Client: If an idiom is identified, provide a link/button to view its details in the Idiom Demystifier.
* [ ] Design UI: Loading indicator while analysis is in progress.
* [ ] Implement Client: Display loading indicator.
* [ ] Design UI: Error message display if analysis fails or returns no specific insights.
* [ ] Implement Client: Handle error responses from the analysis API.
* [ ] Task: Ensure feedback is constructive, educational, and avoids overly prescriptive language.

**User Story 4.3: As a user, I want the text analysis to be reasonably fast for near real-time feedback.**
* [ ] Task (Backend/AI): Optimize AI model inference times for demo cultures.
* [ ] Task (Backend): Optimize backend processing logic for text analysis.
* [ ] Task (Backend): Aim for synchronous processing for typical chat snippets for MVP.
...
* [ ] Design UI: A section or button labeled "Image Analysis (Coming Soon)" or similar.
* [ ] Implement Client: Basic UI element that, when interacted with, shows a message about future availability.
* [ ] *No backend or AI work for this in MVP.*
...
* [ ] Design UI: Screen for Idiom Demystifier with a search bar and a culture selector (defaults to current app culture context, show demo cultures).
* [ ] Implement Client: Input field for search query.
* [ ] Implement Client: Culture selector component.
* [ ] Implement Client: On search, call backend API `GET /cultures/{cultureId}/idioms?search={query}`.
* [ ] Implement Client: Display list of matching idioms (e.g., idiom phrase and brief meaning).
* [ ] Implement Client: Handle "no results found" state.
* [ ] Implement Client: Allow tapping on an idiom from the list to see details.
...
* [ ] Design UI: Detail screen for a single idiom.
* [ ] Implement Client: On selecting an idiom, fetch its full details from `GET /cultures/{cultureId}/idioms/{idiomId}`.
* [ ] Implement Client: Display idiom phrase.
* [ ] Implement Client: Display literal translation (if available).
* [ ] Implement Client: Display actual meaning.
* [ ] Implement Client: Display usage examples.
* [ ] Implement Client: Display context notes (e.g., formality, when to use/avoid).
* [ ] Implement Client: Display politeness level.
* [ ] Implement Client: Display "Last Updated" for the idiom data.
* [ ] Design UI: "Save this Idiom" button (placeholder, functionality deferred for MVP).
...
* [ ] Design UI: Main Settings screen with navigation to sub-sections.
* [ ] Implement Client: Settings screen component.
* [ ] Implement Client: Navigation links to:
    * [ ] Account Management (Logout - from Epic 1. Change Password - placeholder for MVP, Delete Account - placeholder for MVP).
    * [ ] Notification Preferences (placeholder for MVP).
    * [ ] App Language Selection (placeholder for MVP, default to English).
    * [ ] Appearance (Light/Dark Mode) (placeholder for MVP, default to light/system).
    * [ ] Data & Privacy (link to Privacy Policy - from Epic 11).
    * [ ] About (App Version, Credits, Feedback Link - placeholder, Terms of Service Link - from Epic 11).
...
* [ ] Task: Draft a comprehensive Privacy Policy (MVP: simplified version focusing on demo features).
* [ ] Task: Draft Terms of Service (MVP: simplified version).
* [ ] Design UI: Screens to display Privacy Policy and Terms of Service (accessible from Settings and during onboarding).
* [ ] Implement Client: Static display of these legal documents.
* [ ] Task: Ensure Privacy Policy clearly states what data is collected for MVP features, how it's used, stored, and for how long.
